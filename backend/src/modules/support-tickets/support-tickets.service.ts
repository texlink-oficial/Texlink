...
export class SupportTicketsService {
  ...
  async addMessage(
    ...
    if (ticket.status === SupportTicketStatus.FECHADO) {
      throw new ForbiddenException(
        'Não é possível enviar mensagens em um chamado fechado',
      );
    }
    ...
    if (
      ...
      ticket.status !== SupportTicketStatus.RESOLVIDO
    ) {
    ...
    this.logger.log(
      `Emitted ticket.message.added event for ticket ${ticket.displayId}`,
    );
  ...
  async closeTicket(
    ...
    this.logger.log(
      `Emitted ticket.status.changed event for ticket ${ticket.displayId}`,
    );
  ...
  // Get stats (admin)
  async getStats() {
    const [total, abertos, emAndamento, resolvidos, urgentes] =
      await Promise.all([
        this.prisma.supportTicket.count(),
        this.prisma.supportTicket.count({
          where: { status: SupportTicketStatus.ABERTO },
        }),
        this.prisma.supportTicket.count({
          where: { status: SupportTicketStatus.EM_ANDAMENTO },
        }),
        this.prisma.supportTicket.count({
          where: { status: SupportTicketStatus.RESOLVIDO },
        }),
        this.prisma.supportTicket.count({
          where: {
            ...
            status: {
              notIn: [
                SupportTicketStatus.FECHADO,
                SupportTicketStatus.RESOLVIDO,
              ],
            },
          },
        }),
    ...
    const tempoMedioResposta =
      countWithResponse > 0
        ? Math.round(
            (totalResponseTime / countWithResponse / (1000 * 60 * 60)) * 10,
          ) / 10
        : 0;
  ...
  async replyAsSupport(ticketId: string, adminId: string, dto: SendMessageDto) {
    ...
    if (
      !dto.isInternal &&
      ticket.status !== SupportTicketStatus.AGUARDANDO_RESPOSTA
    ) {
