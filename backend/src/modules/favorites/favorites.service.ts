...
export class FavoritesService {
    ...
    private async getBrandCompany(userId: string) {
        ...
        if (!companyUser) {
            throw new ForbiddenException('You must have a brand company to use favorites');
        }
    ...
    async updateFavoriteSupplier(supplierId: string, dto: UpdateFavoriteSupplierDto, userId: string) {
        const company = await this.getBrandCompany(userId);
    ...
    async isFavoriteSupplier(supplierId: string, userId: string): Promise<boolean> {
        const company = await this.getBrandCompany(userId);
    ...
    async updatePaymentPreset(id: string, dto: UpdatePaymentPresetDto, userId: string) {
        const company = await this.getBrandCompany(userId);
