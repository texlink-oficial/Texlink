...
export class CredentialsService {
    ...
    const credential = await this.prisma.supplierCredential.create({
      data: {
        ...
        contactWhatsapp: dto.contactWhatsapp
          ? this.cleanPhone(dto.contactWhatsapp)
          : null,
  ...
  async update(id: string, dto: UpdateCredentialDto, user: AuthUser) {
    ...
    if (dto.contactName !== undefined)
      updateData.contactName = dto.contactName.trim();
    if (dto.contactEmail !== undefined)
      updateData.contactEmail = dto.contactEmail.toLowerCase().trim();
    if (dto.contactPhone !== undefined)
      updateData.contactPhone = this.cleanPhone(dto.contactPhone);
    if (dto.contactWhatsapp !== undefined)
      updateData.contactWhatsapp = dto.contactWhatsapp
        ? this.cleanPhone(dto.contactWhatsapp)
        : null;
    if (dto.tradeName !== undefined)
      updateData.tradeName = dto.tradeName?.trim() || null;
    if (dto.internalCode !== undefined)
      updateData.internalCode = dto.internalCode?.trim() || null;
  ...
  async remove(id: string, user: AuthUser) {
    ...
    this.logger.log(
      `Credenciamento ${id} removido (soft delete) por ${user.id}`,
    );
  ...
  async activateSupplier(credentialId: string, user: AuthUser) {
    ...
    this.logger.log(`Fornecedor ${credentialId} ativado por ${user.id}`);
