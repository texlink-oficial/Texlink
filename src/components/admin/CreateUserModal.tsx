...
import { X, UserPlus, Eye, EyeOff, RefreshCw, ShieldAlert } from 'lucide-react';
  ...
  useEffect(() => {
    if (role === 'BRAND') {
      adminService.getBrands().then((data) => {
        setCompanies(data.map((b: any) => ({ id: b.id, tradeName: b.tradeName || 'Sem nome' })));
      }).catch(() => setCompanies([]));
    } else if (role === 'SUPPLIER') {
      adminService.getSuppliers().then((data) => {
        setCompanies(data.map((s: any) => ({ id: s.id, tradeName: s.tradeName || 'Sem nome' })));
      }).catch(() => setCompanies([]));
    } else {
      setCompanies([]);
      setCompanyId('');
