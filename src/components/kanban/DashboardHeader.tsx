...
import { Bell, Search, Settings, Store, Power, Menu, Star, Moon, Sun, Activity, LogOut } from 'lucide-react';
...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  const workshopName = profile.name || user?.name || 'Minha Facção';
  ...
  return (
    <header className="bg-brand-900 dark:bg-gray-900 border-b border-brand-800 dark:border-gray-800 sticky top-0 z-30 shadow-lg transition-colors duration-300">
            ...
            <div className="hidden md:flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-brand-50">{workshopName}</span>
                    <div className="flex items-center bg-brand-800 dark:bg-gray-800 px-1.5 py-0 rounded border border-brand-700 dark:border-gray-700">
                      ...
                      <span className="text-[10px] font-bold text-yellow-400">{profile.rating.toFixed(1)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col w-32 group cursor-help">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[10px] font-bold text-white">{profile.capacityUsage}%</span>
                </div>
                <div className="h-2 bg-brand-800 dark:bg-gray-700 rounded-full overflow-hidden border border-brand-700 dark:border-gray-600">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${getCapacityColor(profile.capacityUsage)}`}
                    style={{ width: `${profile.capacityUsage}%` }}
                  ></div>
                </div>
              </div>

              <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border ${profile.isActive ? 'bg-green-900/30 text-green-400 border-green-800' : 'bg-red-900/30 text-red-400 border-red-800'}`}>
                <div className={`h-2 w-2 rounded-full ${profile.isActive ? 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.5)]' : 'bg-red-400'}`}></div>
                {profile.isActive ? 'Disponível' : 'Indisponível'}
              </div>
            </div>
          </div>
      ...
      <div className="md:hidden bg-brand-800 dark:bg-gray-800 px-4 py-2 border-b border-brand-700 dark:border-gray-700 flex justify-between items-center text-xs transition-colors">
        <span className="font-semibold text-brand-50 dark:text-gray-200">{workshopName}</span>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <div className="w-10 h-1.5 bg-brand-900 dark:bg-gray-700 rounded-full overflow-hidden border border-brand-700">
              <div className={`h-full ${getCapacityColor(profile.capacityUsage)}`} style={{ width: `${profile.capacityUsage}%` }}></div>
            </div>
            <span className="text-[10px] text-brand-200">{profile.capacityUsage}%</span>
          </div>
          <div className="flex items-center gap-1.5 bg-brand-900/50 dark:bg-gray-900/50 px-2 py-0.5 rounded-full border border-brand-700 dark:border-gray-600">
            <span className={`h-1.5 w-1.5 rounded-full ${profile.isActive ? 'bg-green-400 shadow-[0_0_4px_rgba(74,222,128,0.5)]' : 'bg-red-400'}`}></span>
            <span className="text-[10px] font-medium text-brand-100 dark:text-gray-300">{profile.isActive ? 'Online' : 'Offline'}</span>
          </div>
        </div>
      </div>
    </header>
