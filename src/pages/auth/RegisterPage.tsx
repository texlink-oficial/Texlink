        ...
        <div className="flex items-center justify-center gap-3 mb-6">
            <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-medium transition-colors ${
                    step === 1 ? 'text-white' : 'text-brand-300'
                }`}>
                    Seus Dados
                </span>
            </div>
            ...
            <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${
                    step === 2
                        ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/30'
                        : 'bg-white/10 text-brand-400 border border-white/20'
                }`}>
                    2
                </div>
                <span className={`text-sm font-medium transition-colors ${
                    step === 2 ? 'text-white' : 'text-brand-400'
                }`}>
                    Dados da Empresa
                </span>
            </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-brand-900 via-brand-800 to-brand-950 flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                ...
                <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/20 shadow-2xl">
                    ...
                    {step === 1 && (
                        <div>
                            <h2 className="text-2xl font-semibold text-white mb-6">Seus Dados</h2>

                            {step1Error && (
                                <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3 mb-4">
                                    <p className="text-red-200 text-sm">{step1Error}</p>
                                </div>
                            )}
                                ...
                                <div>
                                    ...
                                    {password.length > 0 && (
                                        <div className="mt-3 space-y-1.5">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${passwordChecks.length ? 'bg-green-500' : 'bg-white/10'}`}>
                                                    {passwordChecks.length && <Check className="w-2.5 h-2.5 text-white" />}
                                                </div>
                                                <span className={`text-xs ${passwordChecks.length ? 'text-green-400' : 'text-brand-400'}`}>
                                                    Pelo menos 8 caracteres
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${passwordChecks.uppercase ? 'bg-green-500' : 'bg-white/10'}`}>
                                                    {passwordChecks.uppercase && <Check className="w-2.5 h-2.5 text-white" />}
                                                </div>
                                                <span className={`text-xs ${passwordChecks.uppercase ? 'text-green-400' : 'text-brand-400'}`}>
                                                    Uma letra maiuscula
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${passwordChecks.lowercase ? 'bg-green-500' : 'bg-white/10'}`}>
                                                    {passwordChecks.lowercase && <Check className="w-2.5 h-2.5 text-white" />}
                                                </div>
                                                <span className={`text-xs ${passwordChecks.lowercase ? 'text-green-400' : 'text-brand-400'}`}>
                                                    Uma letra minuscula
                                                </span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${passwordChecks.number ? 'bg-green-500' : 'bg-white/10'}`}>
                                                    {passwordChecks.number && <Check className="w-2.5 h-2.5 text-white" />}
                                                </div>
                                                <span className={`text-xs ${passwordChecks.number ? 'text-green-400' : 'text-brand-400'}`}>
                                                    Um numero
                                                </span>
                                            </div>
                                        </div>
                    ...
                    {step === 2 && (
                        <div>
                            <h2 className="text-2xl font-semibold text-white mb-6">Dados da Empresa</h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        {cnpjLoading ? (
                                            <Loader2 className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-400 animate-spin" />
                                        ) : (
                                        ...
                                        <input
                                            ...
                                            className={`w-full pl-11 pr-4 py-3 bg-white/5 border rounded-xl text-white placeholder-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all ${cnpjError ? 'border-red-500' : 'border-white/10'} ${isFromInvitation ? 'opacity-60 cursor-not-allowed' : ''}`}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-brand-200 mb-2">
                                        Nome Fantasia
                                        <span className="text-brand-400 font-normal ml-1">(opcional)</span>
                                    </label>
                                    <div className="relative">
                                        <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-400" />
                                        <input
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="col-span-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all"
                                            placeholder="Cidade"
                                        />
                                    </div>
                                    <div>
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        className="flex-shrink-0 py-3 px-4 bg-white/10 hover:bg-white/15 text-white font-medium rounded-xl border border-white/10 transition-all duration-300 flex items-center justify-center gap-2"
                                    >
