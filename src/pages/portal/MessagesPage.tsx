...
import { MessageSquare, Loader2, Package, Search, ChevronRight } from 'lucide-react';
...
const MessagesPage: React.FC = () => {
    ...
    const loadConversations = async () => {
        try {
            setIsLoading(true);
            const orders = await ordersService.getSupplierOrders();

            // Filter orders that have a brand assigned and are active
            const convs: Conversation[] = orders
                .filter((o: Order) => o.brand && o.status !== 'FINALIZADO')
                .map((order: Order) => ({
                    orderId: order.id,
                    displayId: order.displayId,
                    brandName: order.brand?.tradeName || 'Marca',
                    brandImage: order.brand?.logoUrl ?? undefined,
    ...
    const filteredConversations = conversations.filter(
        (conv) =>
            conv.brandName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ...
                    {filteredConversations.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                            ...
                            <p className="text-sm text-gray-400 dark:text-gray-500">
                                As mensagens aparecerão aqui quando você tiver pedidos com marcas.
                            </p>
                        </div>
                    ) : (
                                        ...
                                        {conv.brandImage ? (
                                            ...
                                            <div className="w-12 h-12 rounded-full bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center">
                                                <span className="text-brand-600 dark:text-brand-400 font-bold text-lg">
                                                    {conv.brandName.charAt(0).toUpperCase()}
                                                </span>
                                            </div>
                                        )}
                                        {conv.unreadCount > 0 && (
